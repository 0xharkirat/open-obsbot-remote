...
6. **UI.** The remote is one Live screen (`apps/rc/lib/live_screen.dart`) plus the image controls (`apps/rc/lib/image_controls.dart`) and the mix sequencer (`apps/rc/lib/mix_sequencer_screen.dart`). Add per-camera gating in the relevant section so, for example, virtual background only shows for Meet, or zone-tracking only shows for Tiny SE.

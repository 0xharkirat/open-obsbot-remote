...
Fade-from-black rides on the active stream only.
When `device.set_active` starts a fade, `DeviceManager::active_fade_factor()` ramps from 0 to 1 over the fade window, and the MJPEG server multiplies each active-stream frame's RGB by that factor via `jpeg_darken`, so OBS sees the fade baked into the pixels.

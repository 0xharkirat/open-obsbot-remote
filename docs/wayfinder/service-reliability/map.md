...
- **Chrome may eventually refuse to let the page reach the bridge at all.** Found while researching installability for the [Mobile Operator Experience map](../mobile-ux/map.md). Chrome 142 ships [Local Network Access](https://developer.chrome.com/blog/local-network-access), a permission gate on requests into the local network, and it **restricts the ability to REQUEST that permission to secure contexts**. Our page is served plain HTTP from the LAN, so it could never ask. Today we are out of scope, because the page at `:8765` fetching MJPEG from `:8766` is local-to-local rather than public-to-local. Chrome states an intention to *"extend these protections to cover all cross-origin requests going to destinations on the local network"*, and if that lands as written, the preview breaks with no recourse from the app side. Same-origin proxying of the MJPEG through `:8765` would likely dodge it and is worth pricing early. This is the first argument for HTTPS that is about the product continuing to work rather than about installability, and it is on someone else's schedule, which is what makes it worth watching.

...
- **Whether anything else still reaches the internet.** The CanvasKit fix removed the one that broke the product outright, but the audit was not exhaustive. Known and unfixed: CanvasKit's `fontFallbackBaseUrl` points at `fonts.gstatic.com` and downloads fallback fonts on demand, so a preset or sequence named in Gurmukhi would not render offline. A LAN-only product should be able to state, and ideally test, that it makes zero outbound requests. Deciding how far to take that, and whether to self-host the Noto fallbacks, is not yet sharp.

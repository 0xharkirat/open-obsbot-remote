...
A decided interaction design for **the web app in a mobile browser**: the end-to-end flow a volunteer moves through, from opening the URL to the service being over.

**Amended twice on 2026-08-02.** The original destination was "shipped as a genuinely installable app". Ticket 01 proved that is impossible on Android and only likely on iOS, and the operator then ruled installability out of scope entirely. **A browser tab is the target, not a fallback.** No manifest work, no install prompt, no Add to Home Screen teaching. The tab is simply made good enough that none of that is missed.

That is a simplification rather than a reduction. It removes a per-platform fork from every journey this map decides, and it means one flow serves both phones.
...
| [Decide the Live screen layout](tickets/02-live-screen-layout.md) | grilling, HITL | frontier - **structure decided by 06, composition open** |
| [Decide how the operator knows the system is healthy](tickets/03-health-visibility.md) | grilling, HITL | frontier - **rules decided by 06, vocabulary open** |
| [Decide the pairing and first-run journey](tickets/04-pairing-first-run.md) | grilling, HITL | frontier - **largely answered by 06 Stage 1** |
| [Decide the install journey, per platform](tickets/05-install-journey.md) | grilling, HITL | **retired - out of scope** |
| [Chart the end-to-end mobile browser flow](tickets/06-mobile-browser-flow.md) | spec | frontier |
...
- **Installability retired entirely.** After ticket 01 came back split (iOS likely, Android impossible), the operator ruled it out of scope rather than build a per-platform fork. Ticket 05 is retired. The two things worth keeping from it moved into ticket 06: tab hardening became browser-layer hazards, and "how do I get back here next week" became Stage 0, whose answer is a stable mDNS name rather than a home screen icon.
- **[The end-to-end mobile browser flow](tickets/06-mobile-browser-flow.md), charted.** Six stages plus a browser-hazard layer, grounded in the four research passes and in the code as read on 2026-08-02. Load-bearing decisions: the operating surface does not scroll (two planes, pinned preview and action bar), the big preview carries a permanent PREVIEW or ON AIR badge including the single-camera case, TAKE never shares its row with a control that moves, and a dropped socket degrades in place instead of navigating to the Connect screen. Two verified live defects fall out of it with one-line fixes: pull-to-refresh reloads the app mid-shot, and the Chrome address bar is tinted Flutter-scaffold blue.

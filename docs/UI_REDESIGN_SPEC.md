...
> **Status: shipped, with revisions.** This was the original 5-tab plan. The
> shipped UI is **3 tabs** after live-review feedback collapsed Presets and
> Sequence into other surfaces. See the "Post-review revision" section
> below for what actually landed; the rest of the doc is preserved as
> the design log.

## Post-review revision (final v1.2 layout)

Live tests on a real phone changed three things about the original plan:

1. **The Presets tab was dropped.** Users wanted to save and recall
   while still holding the gimbal control, not switch tabs first.
   Presets are inlined as a P1 to P6 row at the bottom of the Joystick
   and Buttons tabs. Tap recalls with the current move duration;
   long-press opens Save here / Recall instantly / Rename.
2. **The Sequence tab was dropped.** Sequence is now an AppBar action
   with a timeline icon. Tapping it opens the inline timeline editor
   in a route. The editor itself uses the same step-card layout this
   spec described.
3. **The two main tabs ended up visually identical.** Same template
   on Joystick and Buttons so muscle memory carries between them:
   ```
   [Recenter] [Sleep] [Wake]              <- quick actions
      [primary control]  [zoom slider]    <- gimbal + zoom
   [P1 P2 P3 P4 P5 P6]                    <- inline presets
   [Instant 1s 5s 15s 30s 1m 3m 5m]       <- duration chips
   ```

Net structure is 3 tabs: **Joystick / Buttons / Image**.

A few other deltas relative to the original spec:

- The status chip bar above the preview was removed. Pan / Tilt is in
  the preview overlay (live readout), zoom shows next to its slider,
  AI mode lives on the Image tab's Auto-track segmented, FOV on the
  View segmented, run status on the bridge tray icon glyph.
- The Image tab gained per-section Reset buttons next to each section
  header (View / Exposure / Anti-flicker / White balance / Color),
  plus an inline reset icon on every color slider. Matches the
  OBSBOT Center workflow.
- "Center alignment lines" in the grid overlay became an "Attitude
  indicator (steer to align)". The cross translates with yaw / pitch
  and rotates with roll, like an aircraft attitude indicator. To
  re-center the camera, steer the moving cross onto the static
  crosshair.
- The advanced-mode AppBar's Move-duration popup was removed because
  the chip strip at the bottom of every tab already covers it.
  Simple mode keeps its own popup (no chips there).
- Image tab labels shortened so the row fits at 360 to 390 px wide:
  "Auto-expose for face" -> "Face exposure", "Focus on face" ->
  "Face focus", "Flip horizontal" -> "Flip", "Auto white balance" ->
  "Auto WB".

The original PR sequence (A through K, below) was condensed in
practice into a single PR (`fix/ui-revamp-from-review`) after live
testing made it clear the 5-tab structure needed a structural change,
not another fix-up. The PR sequence stays here as a design log.
